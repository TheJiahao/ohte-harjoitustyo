...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiisen_summan_lataaminen_ei_muuta_saldoa(self):
        self.kortti.lataa_rahaa(-100)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_kortilla_voi_syoda_edullisesti_kun_saldoa_on_edullisen_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_kortilla_voi_syoda_maukkaast_kun_saldoa_on_maukkaan_verran(self):
